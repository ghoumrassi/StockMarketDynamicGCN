...
from researchproject.src import NY_FOLDER, COMPANY_NAME_MAPPING, NYT_ADJ_MATRIX, NYT_ENTITY_COUNT_MAP
...
def get_entity_list(start_date=None):
    ...
    c = Counter(orgs)
    with open(NYT_ENTITY_COUNT_MAP, 'wb') as fn:
        pickle.dump(c, fn)
...
def get_official_names(name_list):
    names = []
    for name in name_list:
        try:
            company_name = COMPANY_NAME_MAPPING[name]
...
class NYTFormatter:
    def __init__(self, data_folder):
        ...
        self.num_files = len([f for f in data_folder.iterdir()])
    ...
    def make_summaries(self, load_limit=None):
            ...
            for i, fn in enumerate(self.data_folder.iterdir()):
                ...
                for item in data:
                    ...
                    for kw in keywords:
                        if kw['name'] == 'organizations':
                            orgs.append(kw['value'])
                    self.article_summaries.append([date, section, orgs])
                pbar.update()
                if load_limit:
                    if i == load_limit:
                        break
    ...
    def make_frame(self):
        ...
        df = pd.DataFrame(self.article_summaries, columns=['Date', 'Section', 'Organisations'])
